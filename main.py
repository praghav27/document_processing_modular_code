...
class DocumentProcessorMain:
    ...
    async def process_multiple_documents_rfi(self, uploaded_files: List) -> List[Dict[str, Any]]:
        ...
        print(f"Azure Table Storage: ENABLED - Metadata and components will be stored")
        ...
        print(f"{'='*80}")
        print(f"Phase 1 (Parallel Azure DI with Rate Limiting): {phase1_time:.2f}s")
        print(f"Phase 2 (Parallel Content Extraction with Rate Limiting): {phase2_time:.2f}s")
    ...
    def _print_all_chunks_to_terminal(self, all_chunks: List[Dict]):
        ...
        print(f"All metadata and components stored in Azure Table Storage")
        ...
        for project_id, chunks in chunks_by_project.items():
            ...
            print(f"Table Storage: FileMetadataV2 + ComponentDataV2 records created")
            print(f"{'-'*80}")
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                if components:
                    print(f"  Components ({len(components)}) - Stored in ComponentDataV2:")
                    ...
                    print(f"  Components: None identified")
                
                print(f"  Created: {chunk.get('metadata', {}).get('created_at', 'N/A')}")
                print(f"  Table Storage: Metadata and component records created")
        
        print(f"\n{'='*100}")
        print(f"TERMINAL OUTPUT COMPLETE - All {len(all_chunks)} chunks displayed")
        print(f"AZURE TABLE STORAGE - All metadata and components stored for structured querying")
