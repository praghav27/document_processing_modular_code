...
class DocumentMetadataPrompts:
    ...
    def get_rfi_extraction_prompt(document_text: str):
        """
        Get the RFI metadata extraction prompt for 8 fields (NO CHUNKING VERSION)
...
1. **project_name**: Find the main project name, RFI title, or project identifier. Look for project titles, names, or main subject headings in headers, titles, and document names.

2. **client**: Identify the organization or client requesting the information. Look for "Client:", "Owner:", "Issued by:", issuing organization names, or companies requesting the RFI.
...
3. **region**: Extract geographical location information. Look for:
   ...
   - Cities: Toronto, Calgary, Vancouver, Montreal, Ottawa, etc.
   - Project locations: "Located in...", "Project site in...", "Service area..."
   - Regional identifiers: Northern Ontario, Southern Alberta, Coastal BC, etc.

4. **industry**: Determine the industry sector this RFI relates to. Look for industry indicators and select the most appropriate:
   - "Power & Energy" (electrical systems, power generation, transmission, distribution)
   - "Water & Wastewater" (water treatment, municipal water systems, environmental)
   - "Transportation" (roads, bridges, transit, airports, rail)
   - "Oil & Gas" (petroleum, natural gas, pipelines, refineries)
   - "Mining" (mineral extraction, mining operations, resources)
   - "Environmental" (environmental consulting, remediation, assessments)
   - "Infrastructure" (general infrastructure, municipal, civil works)
   - "Telecommunications" (telecom, communications, networks)
   - "Manufacturing" (industrial, production facilities)
   - "Healthcare" (hospitals, medical facilities)

5. **prepared_date**: Extract document creation date, preparation date, or issue date. Look for "Date:", "Issued:", "Prepared:", "Document Date:", etc. Use YYYY-MM-DD format when possible.

6. **station_discipline**: Identify the primary engineering discipline or technical area. Look for discipline indicators:
   - "Civil" (civil engineering, site work, foundations)
   - "Electrical" (electrical systems, power, instrumentation)
   - "Mechanical" (mechanical systems, HVAC, machinery)
   - "Structural" (structural engineering, buildings, bridges)
   - "Environmental" (environmental engineering, assessments)
   - "Process" (process engineering, chemical processes)
   - "Instrumentation" (control systems, automation)
   - "Control Systems" (SCADA, automation, controls)
   - "HVAC" (heating, ventilation, air conditioning)
   - "Piping" (piping systems, plumbing)
   - "Architectural" (building design, architecture)

7. **scope_of_work**: Create a SHORT summary (maximum 200 words) of the work scope for vector/keyword search. Include:
   - Main activities and deliverables mentioned
   - Key objectives and goals
   - Primary work components
   - Technical requirements
   Focus on searchable keywords and key concepts.

8. **required_activities**: Create a SHORT summary (maximum 200 words) of required activities for vector/keyword search. Include:
   - Specific tasks and responsibilities
   - Actions needed from respondents
   - Deliverable requirements
   - Response expectations
   Focus on actionable items and requirements.
...
{{
    ...
    "scope_of_work": "short summary of work scope (max 200 words) or 'Not Specified'",
    "required_activities": "short summary of required activities (max 200 words) or 'Not Specified'"
}}
...
**CRITICAL**: Return ONLY the JSON object. No explanation, no markdown, no extra text.
    ...
    def get_duration_extraction_prompt(full_document_text: str) -> str:
        """
        Get the duration extraction prompt for RFI documents (DEPRECATED - not used in new flow)
...
Extract the duration now (return ONLY the duration text):
    ...
    def get_rfi_description_prompt(full_document_text: str) -> str:
        """
        Get the RFI description generation prompt (DEPRECATED - not used in new flow)
