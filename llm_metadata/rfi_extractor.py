...
class RFIExtractor:
    ...
    def __init__(self):
        ...
        self.verbalizer = ContentVerbalizer()  # Reuse existing client
        self.new_rfi_fields = [
            'project_name', 'client', 'industry', 'region', 'prepared_date', 'components', 'field_type','voltage_class'
        ]
...
9. **contract_types** : Extract the contract type 
...
Extract the metadata now:
    ...
    def _validate_rfi_metadata(self, metadata: Dict) -> Dict:
        ...
        # Validate basic fields
        for field in ['project_name', 'client', 'industry', 'region', 'prepared_date','field_type','voltage_class']:
            value = metadata.get(field, '')
            if isinstance(value, str):
                value = value.strip()
                # Clean up common placeholder values
