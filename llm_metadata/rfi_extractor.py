...
class RFIExtractor:
    ...
    def __init__(self):
        ...
        self.valid_components = {
            'ELE': {
                'name': 'Electrical',
                'specific_fields': ['disconnect_switches', 'transformer']
            },
            'FND': {
                'name': 'Foundations', 
        ...
        log_custom_event(
                 logger,
                 "Enhanced RFI Extractor initialized - ENHANCED COMPONENT FORMAT WITH SPECIFIC FIELDS + Rate Limiting",
                 level="info",
                )
    ...
    async def extract_metadata_only(self, text_elements: List[Dict], azure_di_result=None) -> Dict:
        ...
        async with RFIExtractor._rate_limiter:
            ...
            for attempt in range(3):  # 3 retry attempts
                try:
                    #print(f"🔍 Enhanced RFI metadata extraction (Rate limited - Attempt {attempt + 1}/3)")
                    log_custom_event(
                                    logger,
                                    f"Enhanced RFI metadata extraction (Rate limited - Attempt {attempt + 1}/3)",
                                    level="info",
                                    )
                    ...
                    #print(f"⏰ Timeout during LLM extraction attempt {attempt + 1}/3")
                    RFIExtractor._request_tracker['timeout_count'] += 1
                    if attempt == 2:
                        #print("❌ All retry attempts timed out. Returning default metadata.")
                        return self._get_rfi_default_metadata()
                    await asyncio.sleep(1.0)  # Wait before retry
                except Exception as e:
                    #print(f"❌ Error extracting enhanced RFI metadata (Attempt {attempt + 1}/3): {e}")
                    RFIExtractor._request_tracker['error_count'] += 1
                    if attempt == 2:
                        #print("❌ All retry attempts failed. Returning default metadata.")
                        return self._get_rfi_default_metadata()
                    await asyncio.sleep(1.0)  # Wait before retry
...
6. **contract_types** : Extract the contract type from the provided RFI document and
     assign it to one of the following standardized contract types only:
    (i) Fixed Price
    (ii) T&M (Time and Materials)
    (iii) MSA (Master Service Agreement)
 
    Use the following synonyms, indicators and related terms as guidance for the respective contract types:
    Fixed Price : lump sum, fixed fee, turnkey, all-inclusive, single price, fixed cost, flat rate, one-time payment, flat fee, firm-fixed.
    T&M (Time and Materials) : hourly rate, labor plus materials, cost-plus, rate card, time-based billing, pay-as-you-go, labor hour, T&M, NTE, materials included.
    MSA (Master Service Agreement) : framework agreement, blanket agreement, umbrella contract, standing offer, base agreement, master contract, service framework, standing agreement.
 
 NOTE :
 - Read the document and scan for keywords plus contextual clues associated with the indicators mentioned above.
...
- "ELE" → Electrical (fields: disconnect_switches, transformer)
...
- **disconnect_switches**: Extract the switches disconnected mentioned (e.g. "115kV isolating switches","230kV line disconnect switches with ground","27.6kV feeder disconnect switches"). Look for disconnect switches in electrical equipment descriptions. If not found: "not mentioned in document"
...
- ****: Extract bus system information (e.g., "Install new 2x2303kcmil ASC drops from high-level bus to breaker disconnect switch"). Look for bus installation or modification work. If not found: "not mentioned in document"
...
7. **field_type**: Extracting the Field Type from all content carefully and identify whether the described 
project or scope of work refers to:

    Green Field : creation of entirely new infrastructure, facilities, systems, 
    or equipment (e.g., new build, new setup, greenfield project, new construction,
    first-time implementation, deployment from scratch).

    Brown Field : modification, alteration, or upgrade of existing infrastructure, facilities,
    systems, or equipment (e.g., refurbishment, replacement, retrofitting, repurposing, 
    expansion, modernization, adjustment to existing setup).

Use the following synonyms and related terms as guidance:

    Green Field synonyms/indicators: new setup, new creation, new construction, greenfield project, 
    fresh installation, full build-out, new deployment, establishing from scratch, first-time implementation.

    Brown Field synonyms/indicators: modification, update, upgrade, refurbishment, retrofit, extension, 
    redevelopment, repurposing, replacement, modernization, alteration, enhancement, expansion.


If the document does not provide enough context to clearly decide whether it is Green Field or Brown Field,
output:"Not mentioned in the document"

NOTE : Your output must always be one of the following three values only:
(i) Green Field
(ii) Brown Field
(iii) Not mentioned in the document
 
8. **voltage_class** : Extract all voltage values mentioned in the provided document text, and label 
them into one of the following standardized voltage classes only:

    (i) Distribution (120 V - 34.5 kV)
    (ii) Sub-Transmission (46–72.5 kV)
    (iii) Transmission (115–230 kV)
    (iv) EHV (Extra High Voltage, 315–765 kV)
    (v) HVDC (±250 kV to ±800 kV)
...
9. **contract_types** : Extract the contract type from the provided RFI document and
     assign it to one of the following standardized contract types only:
    (i) Fixed Price
    (ii) T&M (Time and Materials)
    (iii) MSA (Master Service Agreement)
...
{{
   ...
   "voltage_class" : "extracted voltage class or Not mentioned in the document",
   "contract_types": "extracted contract type or Not mentioned in the document",
   "pricing": "extracted pricing info or not mentioned in document",
   "components": {{
     "ELE": {{
       ...
       "disconnect_switches": "switches disconnected or not mentioned in document",
...
Extract the metadata now:
    ...
    def _validate_rfi_metadata(self, metadata: Dict) -> Dict:
        ...
        for field in ['project_name', 'client', 'industry', 'region', 'prepared_date','field_type','voltage_class','contract_types','pricing']:
