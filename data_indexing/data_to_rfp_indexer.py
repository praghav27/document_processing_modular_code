...
class AzureSearchRFPResponseUploader:
    @tracer.start_as_current_span("AzureSearchRFPResponseUploader_init_fn")
    def __init__(self):
        load_dotenv()
    ...
    @tracer.start_as_current_span("transform_chunk_to_document_fn")
    def transform_chunk_to_document(self, chunk: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "submission_date" in llm_metadata:
            llm_metadata["submission_date"] = self.parse_date(
                llm_metadata["submission_date"]
            )
    ...
    @tracer.start_as_current_span("generate_embeddings_for_chunks_fn")
    def generate_embeddings_for_chunks(self, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if content_text:
                ...
                log_custom_event(
                 logger,
                 f"No content found for chunk {chunk.get('chunk_id', 'Unknown')}",
                 level="warning",
                )
    ...
    def upload_documents_in_batches(self, documents: List[Dict[str, Any]], batch_size: int = 50):
        ...
        log_custom_event(
                 logger,
                 f"Upload completed! Total documents: {total_docs}, Successful uploads: {successful_uploads}, Failed uploads: {failed_uploads}",
                 level="info",
                )
    ...
    def upload_chunks_from_dict(self, chunk_data: Dict[str, Any]):
        ...
        if not chunks:
            ...
            log_custom_event(
                 logger,
                 "No chunks found in the input data",
                 level="warning",
                )
            return
        ...
        log_custom_event(
                 logger,
                 "Generating embeddings and transforming documents...",
                 level="info",
                )
    ...
    def load_and_upload_chunks(self, json_file_path: str):
        try:
            ...
            log_custom_event(
                 logger,
                 f"Loaded JSON data from {json_file_path}",
                 level="info",
                )
