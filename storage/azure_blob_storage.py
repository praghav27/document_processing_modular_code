...
class AzureBlobStorage:
    ...
    @tracer.start_as_current_span("AzureBlobStorage_init_fn")
    def __init__(self):
        self.blob_service_client = BlobServiceClient.from_connection_string(AZURE_STORAGE_CONNECTION_STRING)
        self.input_container = INPUT_CONTAINER_NAME
        self.output_container = OUTPUT_CONTAINER_NAME
    ...
    @tracer.start_as_current_span("set_project_context_fn")
    def set_project_context(self, filename: str, document_type: str):
        """Set project ID and document type from filename"""
        # Extract first 15 characters as project_id
        self.project_id = filename[:15] if filename else "unknown"
        ...
        log_custom_event(
                        logger,
                        f"Storage context set: {self.project_id}/{self.document_type}",
                        level="info",
                        )
    ...
    @tracer.start_as_current_span("save_table_fn")
    def save_table(self, df: pd.DataFrame, filename: str, table_index: int) -> str:
        """Save table as CSV to blob"""
        self._ensure_project_context()
        csv_filename = f"{filename}_table_{table_index}.csv"
        ...
        try:
            ...
            log_custom_event(
                            logger,
                            f"Saved table {table_index} to blob: {blob_path}",
                            level="info",
                            )
            return blob_path
        except Exception as e:
    ...
    @tracer.start_as_current_span("save_figure_image_bytes_fn")
    def save_figure_image_bytes(self, image_bytes: bytes, filename: str, figure_index: int) -> str:
        """Save figure image from raw bytes to blob"""
        self._ensure_project_context()
        try:
            ...
            log_custom_event(
                            logger,
                            f"Saved figure image to blob: {blob_path}",
                            level="info",
                            )
            return blob_path
        except Exception as e:
    ...
    @tracer.start_as_current_span("save_figure_text_fn")
    def save_figure_text(self, text_content: str, filename: str, figure_index: int) -> str:
        """Save figure text content to blob"""
        self._ensure_project_context()
        txt_filename = f"{filename}_figure_{figure_index}.txt"
        ...
        try:
            ...
            log_custom_event(
                            logger,
                            f"Saved figure text to blob: {blob_path}",
                            level="info",
                            )
            return blob_path
        except Exception as e:
    ...
    @tracer.start_as_current_span("save_text_chunks_fn")
    def save_text_chunks(self, text_chunks: List[Dict], filename: str) -> tuple:
        """Save text chunks as JSON to blob - Returns (chunk_data, blob_path)"""
        self._ensure_project_context()
        json_filename = f"{filename}_text_chunks.json"
        ...
        try:
            ...
            log_custom_event(
                            logger,
                            f"💾 Saved text chunks to blob: {blob_path}",
                            level="info",
                            )
            if llm_metadata:
                #print(f"   🤖 LLM Metadata: Project='{llm_metadata.get('project_title', 'N/A')[:30]}...', Vendor='{llm_metadata.get('vendor_name', 'N/A')}', Domain='{llm_metadata.get('domain_category', 'N/A')}'")
    ...
    @tracer.start_as_current_span("save_text_chunks_RFI_fn")
    def save_text_chunks_RFI(self, text_chunks: List[Dict], filename: str) -> tuple:
        """Save enhanced RFI text chunks with verbalization and LLM metadata as JSON file - Returns (chunk_data, json_path)"""
        self._ensure_project_context()
        json_filename = f"{filename}_text_chunks.json"
        blob_path = f"{self.project_id}/{self.document_type}/text/{json_filename}"
        ...
        chunk_data = {
            "filename": filename,
            "total_chunks": len(text_chunks),
            "processing_method": "enhanced_rfi_parallel_processing_with_components",
            "llm_extracted_metadata": llm_metadata,  # Store LLM metadata at document level
            "chunk_types": {
                "text": len([c for c in text_chunks if c.get('content_type') == 'text']),
                "table": 0,  # RFI doesn't process tables/images as chunks
                "image": 0   # RFI doesn't process tables/images as chunks
            },
            "created_at": text_chunks[0].get("metadata", {}).get("created_at", "") if text_chunks else "",
            "chunks": []
        }
        ...
        for chunk in text_chunks:
            chunk_info = {
                ...
                "chunk_id": chunk.get("chunk_id", ""),
                "project_id": chunk.get("project_id", ""),
                "components": chunk.get("components", {}),  # Store full components structure
                "metadata": chunk.get("metadata", {})
            }
            chunk_data["chunks"].append(chunk_info)
        ...
        try:
            ...
            # print(f"💾 Saved RFI chunks to blob: {blob_path}")
            # print(f"   📊 Total chunks: {chunk_data['total_chunks']}")
            # print(f"   🎯 Processing method: {chunk_data['processing_method']}")
            log_custom_event(
                            logger,
                            f"Saved RFI chunks to blob: {blob_path}",
                            level="info",
                            )
            if llm_metadata:
                project_name = llm_metadata.get('project_name', 'N/A')[:50]
                components_count = len(llm_metadata.get('components', {}))
                #print(f"   🤖 LLM Metadata: Project='{project_name}...', Components={components_count}")
            
            return chunk_data, blob_path
        except Exception as e:
            #print(f"❌ Error saving RFI text chunks: {e}")
            return None, None

    @tracer.start_as_current_span("save_raw_text_fn")
    def save_raw_text(self, raw_text: str, filename: str) -> str:
        """Save raw extracted text to blob"""
        self._ensure_project_context()
        txt_filename = f"{filename}_raw_text.txt"
        ...
        try:
            ...
            log_custom_event(
                            logger,
                            f"Saved raw text to blob: {blob_path}",
                            level="info",
                            )
            return blob_path
        except Exception as e:
