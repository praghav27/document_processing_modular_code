...
from processors.extraction.text_extractor import TextExtractor #src.document_process.
...
import time
from config import ( #src.document_process.
...
from llm_metadata.rfi_extractor import RFIExtractor #src.document_process.
from azure.data.tables import TableServiceClient, TableClient
from azure.core.exceptions import ResourceNotFoundError

from application_logging.custom_logging_to_app_insights import configure_logger, log_custom_event #src.document_process.
...
@tracer.start_as_current_span("query_azure_table_async_fn")
...
# 🔌 OPENAI CLIENT
# =========================
@tracer.start_as_current_span("get_openai_client_fn")
...
async def extract_rfi_chunk_from_file_parallel(file_path: str, file_name: str, project_id: str = None) -> Dict:
    ...
    print(f"here is the chunk{chunk}")
    return chunk
...
def create_enhanced_chunk(metadata: Dict, text_elements: List, file_name: str, project_id: str = None) -> Dict:
    ...
    # Save all_text to a text file
    all_text_filename = f"{file_name}_all_text.txt"
    with open(all_text_filename, "w", encoding="utf-8") as f:
        f.write(all_text)
    print(f"all_text saved to {all_text_filename}")
...
COMPONENT_SEARCH_FIELDS = {
    "electrical":                  ["pricing", "scope_of_work", "required_activities", "electrical_discipline_summary"],
    "equipment":                   ["pricing", "scope_of_work", "required_activities", "equipment_discipline_summary"],
    "auxiliary":                   ["pricing", "scope_of_work", "required_activities", "auxiliary_discipline_summary"],
    "line":                        ["pricing", "scope_of_work", "required_activities", "line_discipline_summary"],
    "site_preparation":            ["pricing", "scope_of_work", "required_activities", "site_preparation_discipline_summary"],
    "access_roads":                ["pricing", "scope_of_work", "required_activities", "access_roads_discipline_summary"],
    "drainage":                    ["pricing", "scope_of_work", "required_activities", "drainage_discipline_summary"],
    "undergrounds":                ["pricing", "scope_of_work", "required_activities", "undergrounds_discipline_summary"],
    "environment":                 ["pricing", "scope_of_work", "required_activities", "environment_discipline_summary"],
    "foundations":                 ["pricing", "scope_of_work", "required_activities", "foundations_discipline_summary"],
    "substation_structures":       ["pricing", "scope_of_work", "required_activities", "substation_structures_discipline_summary"],
    "buildings":                   ["pricing", "scope_of_work", "required_activities", "buildings_discipline_summary"],
    "firewalls_and_barriers":      ["pricing", "scope_of_work", "required_activities", "firewalls_and_barriers_discipline_summary"],
    "line_structures":             ["pricing", "scope_of_work", "required_activities", "line_structures_discipline_summary"],
    "protection_and_control":      ["pricing", "scope_of_work", "required_activities", "protection_and_control_discipline_summary"],
    "metering":                    ["pricing", "scope_of_work", "required_activities", "metering_discipline_summary"],
    "telecom_and_teleprotection":  ["pricing", "scope_of_work", "required_activities", "telecom_and_teleprotection_discipline_summary"],
}
...
# =========================
@tracer.start_as_current_span("process_multiple_files_parallel_enhanced_fn")
...
async def main():
    ...
    input_qn = "What are the assumptions for the Eng Study-RFP Document?"
    
    file_list = [
         #r"C:\Users\jhagan.a\Documents\Tetratech\RPFRFIs\five_splitted\705-22295407.00-B&M_HONI RFP-Chatham SS-Chatham_Lakeshore App AI Elect Install Works 1\705-25795001.00-NL Hydro RFP-LIL Eng Study-RFP Document.pdf"
         #r"C:\Users\jhagan.a\Downloads\t2.docx"
         r"C:\Users\Harshal.chaudhari\venvs\Tetra_Tech_ZIP\tetra_Ttech_proposal_documents\five_splitted_document\CET 2025 EPC RFP_091925-Issued 1 (2) 1.pdf",
         #r"C:\Users\jhagan.a\Documents\Tetratech\RPFRFIs\five_splitted\705-22295407.00-B&M_HONI RFP-Chatham SS-Chatham_Lakeshore App AI Elect Install Works 1\705-25795001.00-NL Hydro RFP-LIL Eng Study-Tower Design Criteria.pdf"
         ]
...
"""
    ...
    def call_openai():
        response = client.chat.completions.create(
            ...
            # temperature=0.3,
            max_completion_tokens=8000,  # Increased for multi-document response
        )
        return response.choices[0].message.content.strip()
